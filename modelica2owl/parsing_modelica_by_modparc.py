...
def raiseifnot(cond, msg="", exc=AssertionError):
    if not cond:
        raise exc(msg)
class P():
    def __init__(self, logg = False):
        self.logg = logg
    def __lt__(self,other):
        if not self.logg: print(other)
        else: logging.debug(str(other))
p=P(); l=P(logg=True)
...
# p<remove_all_inside_function("aklnda sklfjlkasdf \n asfjksfj \n start\nfur(\nstart ansdlkna aksld fur(aksmd) asdkkd ( aijsdj) ksadlk kask\n alksj end); fur(asd);end", "fur")
# s()

def get_description_name(model_definition) -> str:
    try: return model_definition.name()
    except Exception as e: 
        logging.error(model_definition)
        raise TypeError(f"Got {e} when asking for name of model_definition") 
...
def iterate_over_class_definitions(stored_definition, onto, omc: OMCSessionZMQ, curr_imp_path: str = "") -> None:
    # if omc.isPackage(curr_imp_path):
    #     subpackages=parseIsPackegeOutput(omc.getPackages(curr_imp_path))
    #     for subpackage in subpackages:
    #         l<f"curr_imp_path+'.'+subpackage="+curr_imp_path+"."+subpackage
    #         iterate_over_class_definitions(stored_definition, onto, omc, curr_imp_path+"."+subpackage)
    #     return

    for cl_df in stored_definition.search("ClassDefinition"):
        name = get_description_name(cl_df)#cl_df.search("Name")[0] # 
        p<cl_df.class_type() # for sure gets package gets also partial? 
        ...
        l<(1,name, class_definition_code, last_class_prefix)
        ...
        match last_class_prefix:
            case "package":
                ...
                l<("Class definitins:", len(cl_df.search("ClassDefinition")), "---", [get_description_name(x) for x in cl_df.search("ClassDefinition")])
                s()
                iterate_over_class_definitions(cl_df, onto, omc, curr_imp_path)
                continue
            case "model":
                last_df_was_package = False
...
def generate_ontology_from_library(abs_lib_path: str, abs_out_file_path: str, ontology_url: str = MODELICA_EXAMPLE_URL):
    ...
    modelica_code = clean_modelica_code(open(abs_lib_path, "rt", encoding=sys.getfilesystemencoding()).read())
    p<f"parsiing library with modparc..."
    model_definition = modparc.parse(modelica_code)
    file_name = os.path.basename(abs_lib_path)[:-3] 
    assert abs_lib_path[-3:] == ".mo", f"Path does not terminate with '.mo'. Path is {abs_lib_path}"
    library_name = get_description_name(model_definition)
    assert library_name == file_name, f"Actual model name ({library_name}) is different from file name ({file_name})"

    p<f"instantiating ZMQ session..."
    omc = OMCSessionZMQ()
    p<f"loading library file..."
    assert omc.loadFile(abs_lib_path), f"omc.loadFile({abs_lib_path}) did not work: returned {omc.loadFile(abs_lib_path)}"
    raiseifnot(omc.isPackage(library_name), f"{library_name} should be a package!")
    iterate_over_class_definitions(model_definition, ONTO, omc, library_name)
...
def get_total_instatiation_AST(file_path: str, depended_libraries: list[str]) -> dict:
    ...
    model_description = modparc.parse(open(file_path, "rt", encoding=sys.getdefaultencoding()).read())
...
replaceable DataStructures.FlowUnit flowUnit;
    ...
    # s()
    generate_ontology_from_library(library_abs_path, ontology_from_library_abs_path, ontology_url=MODELICA_EXAMPLE_URL)
    ...
    # AST = json.load(open(r"C:\Users\Francesco\Desktop\Work_Units\modelica_constraints\modelica_files\myProcResAST.json", "r", encoding=sys.getdefaultencoding()))
